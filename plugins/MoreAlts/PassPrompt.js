...
function PasswordPrompt({ dialogInfo, onCancel, onSuccess, storage }) {
  ...
  const handlePasswordDialog = async () => {
    if (!passwordInput.trim()) {
      showToast("Please enter the password", 1);
      return;
    }

    const stored = storage.settings.exportPasswordHash;
    const ok = await verifyPassword(passwordInput, stored);
    if (ok) {
      ...
      // Old simpleHash format lacks '$' — tell user to reset
      if (stored && !stored.includes('$')) {
        showToast("Old password format detected — please set a new password in Settings", 1);
      } else {
