...
import { generateEncryptionKey, encryptToken, decryptToken, getDecryptedToken, getEncryptedTokenData } from "./PasswordUtils.js";
	...
	async onLoad() {
		...
		try {
			...
			for (const id of Object.keys(storage.accounts || {})) {
				const account = storage.accounts[id];
				if (account?.token && !account.token.startsWith("enc:")) {
					storage.accounts[id] = { ...account, token: await encryptToken(account.token, encKey) };
				}
			}
		} catch (e) {
			console.error("Token encryption migration failed:", e);
		}
		...
		const commands = {
			...
			async addAccount(args, ctx) {
				try {
					...
					await addHistoryEntry({ action: 'add', username: currentUser.username, accountId: currentUser.id });
			...
			async login(args, ctx) {
				try {
					...
					try {
						const token = await getDecryptedToken(account, storage.settings);
