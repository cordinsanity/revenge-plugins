...
import MessageVaultSettings from "./Settings";
...
function patchTabsUI(tabs, patches) {
    if (!settingConstants || !SettingsOverviewScreen || !tabsNavigationRef) {
        console.warn(
            "[MessageVault] Missing required constants for tabs UI patch",
        );
        return;
    }
    ...
    const firstRender = Symbol("MessageVault-pinToSettings");
...
export default function patchSidebar() {
    if (!storage.settings?.addToSidebar) {
        console.log("[MessageVault] Sidebar disabled in settings");
        return () => {};
    }

    console.log(
        "[MessageVault] Patching sidebar using custom patchSettingsPin...",
    );

    try {
        const unpatch = patchSettingsPin({
            key: "MessageVaultLogger",
            icon: getAssetIDByName("ClockIcon"),
            title: () => "Message Logger",
            predicate: () => storage.settings?.addToSidebar === true,
            page: MessageVaultSettings,
        });

        console.log("[MessageVault] Successfully patched sidebar");
        return unpatch;
    } catch (error) {
        console.error("[MessageVault] Failed to patch sidebar:", error);
        return () => {};
    }
}
